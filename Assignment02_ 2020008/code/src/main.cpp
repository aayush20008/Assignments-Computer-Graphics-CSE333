...
void setupProjectionTransformation(unsigned int &program, char temp);
	...
	while (!glfwWindowShouldClose(window))
	{
        ...
        if (ImGui::IsKeyDown(ImGui::GetKeyIndex(ImGuiKey_LeftArrow))) {
          ...
          strcpy(textKeyStatus, "Key status: Left");
			...
			if(io.KeyCtrl){
			camPosition = glm::vec4(1007.0f, 0.0, 0.0f, 1.0);
		  }
		  else{
			camPosition.x += 3.5f;
          ...
          if(io.KeyShift){
			...
			camPosition = glm::vec4(0.0, 1034.0, 3.5f, 1.0);
		  }
...
void setupProjectionTransformation(unsigned int &program, char temp)
{
	...
	if(temp == 'P'){
		...
		projectionT = glm::ortho(-50.0f,50.0f,-50.0f,50.0f,0.1f, 1000.0f); //left,right,bottom,top,near,far
	}
